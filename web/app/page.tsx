...
import { formatCurrency, formatPercent, readableLabel, runHref, selectRun, visibleRuns } from "@/lib/view-model";

export default async function OverviewPage({ searchParams }: { searchParams: Promise<{ run?: string }> }) {
  ...
  return <div className="page-wrap">
    ...
    <section className="metric-grid">
      <MetricCard label="Match rate" value={formatPercent(run.summary.match_rate)} detail={`${run.summary.matched_records} groups reconciled`} icon={BadgeCheck} />
    ...
    <div className={`overview-grid${run.run_type === "upload" ? " upload-overview" : ""}`}>
      ...
      {run.run_type !== "upload" && <section className="panel"><div className="panel-heading"><div><p className="eyebrow">Control quality</p><h2>Exception accuracy</h2></div><a href={runHref("/reconciliations", run.id)}>Review all</a></div><div className="exception-list">{run.exception_metrics.map((item) => <div key={item.category}><div><strong>{readableLabel(item.category)}</strong><small>{item.count} records</small></div><span>{item.precision === null ? "—" : formatPercent(item.precision)}</span></div>)}</div></section>}
    </div>
  </div>;
}
