...
import { formatCurrency, readableLabel, resultStatus, runHref, selectRun, visibleRuns } from "@/lib/view-model";
...
export default async function ReconciliationsPage({ searchParams }: { searchParams: Promise<{ run?: string; status?: string }> }) {
  ...
  const run = selectRun(shownRuns, params.run);
